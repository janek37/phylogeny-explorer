...
function Image(props: {leaf: Leaf, positioner: positioner, onClick: (nodeId: TreeLeaf) => void}) {
  ...
  return <g className='image' onClick={() => {props.onClick(leaf.value)}}>
    <image href={process.env.PUBLIC_URL + leaf.value.image.local_thumbnail_url} width={size} height={size} x={x - size/2} y={y - size/2}/>
    {leaf.value.extinct ? <image href={`${process.env.PUBLIC_URL}/extinct.svg`} width={size/4} height={size/4} x={x - size/2} y={y - size/2} /> : ''}
